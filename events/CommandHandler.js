  ...
  async chatUpdate(messages) {
    ...
    try {
      ...
      if (isCommand) {
        ...
        // Inisialisasi default values untuk extra
        let participants = [];
        let user = {};
        let bot = {};
        let isAdmin = false;
        ...
        // --- OPTIMASI: Ambil metadata HANYA jika diperlukan ---
        // Hanya ambil metadata untuk command yang memerlukan permission checks
        const needsMetadata = msg.isGroup && (command.admin || command.botAdmin || command.group);
        
        if (needsMetadata) {
          try {
            groupMetadata = await getGroupMetadata(msg.from, this);
            participants = groupMetadata?.participants || [];
            user = participants.find((u) => u.id == msg.sender) || {};
            bot = participants.find((u) => u.id == Serializer.decodeJid(this.user.id)) || {};
            ...
            // Jika gagal ambil metadata, berikan feedback ke user
            if (command.admin || command.botAdmin) {
              return msg.reply("⚠️ Tidak dapat memverifikasi izin saat ini. Silakan coba lagi nanti.");
            }
            
            // Untuk command group biasa, tetap lanjutkan dengan nilai default
            groupMetadata = null;
          }
        }
        ...
        if (command.botAdmin && !isBotAdmin) {
          return msg.reply("⚠️ Bot harus menjadi admin untuk menjalankan perintah ini.");
        }
        ...
        if (command.owner && !isOwner) {
          return msg.reply("⚠️ Perintah ini khusus untuk Owner Bot.");
        }
