...
async function handleTransparentGif(inputPath, outputPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Step 1: Extract frames dari GIF dengan handling transparency
    ffmpeg(inputPath)
      .outputOptions([
      ...
      .on("end", () => {
        // Step 2: Rebuild ke WebP animasi dari extracted frames
    ...
    if (isVideo) {
      ...
      ffmpegCmd = ffmpegCmd
        ...
        .outputOptions([
          ...
          "-vf", "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:-1:-1:color=white@0.0,fps=24", // Filter yang diperbaiki
          "-loop", "0",
          "-an", // No audio
          "-preset", "default",
          "-lossless", "0",
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isVideo) {
      ffmpegCmd = ffmpegCmd
        ...
        .outputOptions([
          "-c:v", "libwebp",
          "-vf", "scale=480:480:force_original_aspect_ratio=decrease,fps=24", // Lebih sederhana
          "-loop", "0",
          "-an",
          "-lossless", "0",
...
async function ensureFileSizeLimit(filePath, maxSize = 950 * 1024, isAnimated = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isAnimated) {
      ...
      ffmpegCmd = ffmpegCmd
        .outputOptions([
          "-c:v", "libwebp_anim",
          "-vf", "scale=480:480:force_original_aspect_ratio=decrease", // Ukuran lebih kecil
          "-loop", "0",
          "-preset", "default",
          "-lossless", "0",
          "-qscale", "30", // Kompresi sangat agresif
          "-compression_level", "6",
          "-method", "6"
        ]);
    } else {
    ...
    if (mediaType === 'animated_webp' || mediaType === 'gif' || mediaType === 'video') {
      ...
      try {
        ...
        if (isTransparentGif) {
          ...
          const finalSize = await handleTransparentGif(properInputPath, tempOutputPath);
          
          if (finalSize > maxSizeBytes) {
            await ensureFileSizeLimit(tempOutputPath, maxSizeBytes, true);
          }
          
          processedBuffer = fs.readFileSync(tempOutputPath);
          console.log(`✅ Transparent GIF processed successfully: ${finalSize} bytes`);
        } else {
          ...
          if (finalSize > maxSizeBytes) {
            finalSize = await ensureFileSizeLimit(tempOutputPath, maxSizeBytes, true);
          }
        ...
        try {
          ...
          if (isTransparentGif) {
            ...
            processedBuffer = fs.readFileSync(tempOutputPath);
            console.log(`✅ Transparent GIF processed with fallback: ${finalSize} bytes`);
          } else {
