...
async function createStickerFromTGS(tgsBuffer, options) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Perbaikan: Gunakan lottieNode dengan cara yang benar
      const lottie = lottieNode.setupNode(animationData);
      ...
      // Gunakan properti animasi yang tersedia
      const totalFrames = animationData.op || 60; // fallback ke 60 frame
      const frameRate = animationData.fr || 30; // fallback ke 30 fps
      const duration = totalFrames / frameRate;

      // Konversi TGS ke WebP menggunakan FFmpeg langsung
      ffmpeg()
        .input(tempTgsPath)
        .inputOptions([
          '-f', 'lottie',
          '-t', Math.min(duration, 3).toString() // batasi durasi maksimal 3 detik
        ])
        .outputOptions([
          '-vcodec', 'libwebp',
        ...
        .on('end', () => {
          try {
            ...
            if (!fs.existsSync(tempWebpPath) || fs.statSync(tempWebpPath).size === 0) {
              throw new Error("Failed to create WebP from TGS");
            }
            ...
            console.error("Error in FFmpeg end callback:", endError);
            reject(endError);
          }
        })
        .on('error', (err) => {
          ...
          // Fallback: Coba konversi sebagai stiker statis
          try {
            // Ekstrak frame pertama sebagai fallback
            const staticSticker = new Sticker(tgsBuffer, {
              ...options,
              background: 'transparent',
              type: StickerTypes.FULL
            });
            console.log("Using static fallback for TGS");
            resolve(staticSticker);
          } catch (fallbackError) {
      ...
      // Fallback terakhir: coba sebagai stiker statis
      try {
        const fallbackSticker = new Sticker(tgsBuffer, {
          ...options,
          background: 'transparent',
          type: StickerTypes.FULL
        });
        console.log("Using final static fallback for TGS");
        resolve(fallbackSticker);
      } catch (fallbackError) {
        reject(err);
      }
    }
