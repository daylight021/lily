...
// FIXED: Simplified and more compatible FFmpeg parameters
async function convertVideoToWebPWithCompression(inputPath, outputPath, metadata, targetSize = 800 * 1024) {
  ...
  // Simplified strategies with more compatible parameters
  const strategies = [
    { quality: 80, resolution: 512, fps: 20 },
    { quality: 70, resolution: 450, fps: 15 },
    { quality: 60, resolution: 400, fps: 12 },
    { quality: 50, resolution: 350, fps: 10 },
    { quality: 40, resolution: 300, fps: 8 },
    { quality: 35, resolution: 280, fps: 6 },
    { quality: 30, resolution: 250, fps: 5 },
    { quality: 25, resolution: 220, fps: 4 },
    { quality: 20, resolution: 200, fps: 3 },
    { quality: 15, resolution: 180, fps: 3 }
  ];
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        const ffmpegProcess = ffmpeg(inputPath)
          ...
          .outputOptions([
            "-vcodec", "libwebp",
            "-vf", `scale=${strategy.resolution}:-1,fps=${strategy.fps}`,
...
// FIXED: Simplified and more compatible FFmpeg parameters for images
async function convertImageToWebPWithCompression(inputPath, outputPath, targetSize = 800 * 1024) {
  console.log(`Converting image, target: ${targetSize} bytes`);
  
  // Simplified strategies with basic, compatible parameters
  const strategies = [
    { quality: 85, resolution: 512 },
    { quality: 75, resolution: 450 },
    { quality: 65, resolution: 400 },
    { quality: 55, resolution: 350 },
    { quality: 45, resolution: 300 },
    { quality: 35, resolution: 280 },
    { quality: 30, resolution: 250 },
    { quality: 25, resolution: 220 },
    { quality: 20, resolution: 200 },
    { quality: 15, resolution: 180 }
  ];
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        ffmpeg(inputPath)
          .outputOptions([
            "-vcodec", "libwebp",
            "-vf", `scale=${strategy.resolution}:-1`,
            "-qscale", strategy.quality.toString(),
            "-y"
          ])
          .save(tempOutput)
...
// FALLBACK: Simple conversion without advanced parameters
async function simpleConvertToWebP(inputPath, outputPath, isVideo = false) {
  console.log("Using fallback simple conversion...");
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpegCmd
      ...
      .on("error", (err) => {
        clearTimeout(timeout);
        console.error("Simple conversion error:", err.message);
        reject(err);
      });
  });
}
...
async function createSticker(mediaBuffer, options = {}) {
  ...
  try {
    ...
    if (mediaType === 'tgs') {
      ...
      try {
        ...
        try {
          ...
          console.log("Advanced conversion failed, trying simple conversion...");
          await simpleConvertToWebP(properInputPath, tempOutputPath, true);
          processedBuffer = fs.readFileSync(tempOutputPath);
      ...
      try {
        if (mediaType === 'webp') {
          ...
          try {
            ...
            console.log("WebP processing failed:", webpError.message);
            throw webpError;
          }
        } else {
          // Handle JPEG and PNG
          try {
            ...
            console.log("Advanced conversion failed, trying simple conversion...");
            await simpleConvertToWebP(properInputPath, tempOutputPath, false);
            processedBuffer = fs.readFileSync(tempOutputPath);
    ...
    if (processedBuffer.length > 950 * 1024) {
