...
async function createStickerFromTGS(tgsBuffer, options) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Logika konversi di sini akan berbeda tergantung pustaka rendering TGS yang Anda pakai
      // Untuk contoh ini, saya asumsikan kita merender setiap frame dan menggabungkannya
      // Ini adalah contoh yang membutuhkan banyak resource
      
      const framePromises = [];
      const imageList = [];
      
      for (let i = 0; i < totalFrames; i++) {
        framePromises.push(new Promise((frameResolve) => {
          lottie.goToAndStop(i, true);
          const framePath = path.join(TEMP_DIR, `frame_${Date.now()}_${i}.png`);
          fs.writeFileSync(framePath, lottie.get,'png');
